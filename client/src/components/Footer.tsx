...
const Footer = () => {
  ...
  // Track scroll position to trigger effects
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <div className="relative">
      {/* White background overlay for scroll effect */}
      <div 
        className={`fixed inset-0 bg-white transition-opacity duration-300 pointer-events-none ${
      ...
      >
        ...
        <motion.button
          className={`absolute right-6 top-10 bg-black border border-white rounded-full p-2 text-white transition-opacity ${isInView ? 'opacity-100' : 'opacity-0'}`}
